...
**Timestamp format:** Agents MUST supply timezone-qualified ISO 8601 timestamps (e.g., `"2026-03-15T18:00:00-05:00"` or `"2026-03-15T23:00:00Z"`) when evaluating booking window constraints. Naive timestamps without UTC offset or Z suffix produce undefined behavior. When `venue_timezone` is present and the agent supplies timezone-qualified timestamps, the booking window can be reliably enforced. The `venue_timezone` field enables agents to interpret venue-local business rules (e.g., "no same-day bookings" means no bookings for today in the venue's timezone). Agents without timezone-aware date libraries SHOULD treat the booking window as informational when timezone conversion is needed.
